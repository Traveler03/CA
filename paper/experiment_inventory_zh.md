...
核验仓库：[Traveler03/CA][repo]。原核验快照为 `main` 的 `bda6641d818db682ba95e7dee024b04f7c864aa2`；本次补充核对 `rag-baselines-qwen3` 分支的 `cb0d1b2a3ae9a8523264f814a3c960f5e256809f`。来源链接分别固定到对应提交。
...
| 类别 | 当前可确认内容 | 状态与计数方式 |
| --- | --- | --- |
| 主实验 | 3 个答题模型，12 个模型与方法配置 | 已归档实测，见第 3 节 |
| Qwen 运行阶段消融 | 完整 CA + 6 个变体 | 已归档实测；完整 CA 与主实验 M03 相同，不重复计数 |
| 主实验与消融合计 | 18 个不重复的低资源评测配置 | 每个配置覆盖 47,815 个实例；这是配置数，不是 18 次独立重复或多随机种子实验 |
| 高资源语言补充评测 | Qwen CA 的 Global-MMLU/en、MMLU-ProX/en | 已归档实测，不混入五语言主实验均值 |
| D-RAG 历史对照 | 分支归档另有 1 个 D-RAG adaptive 配置，见第 5.6 节 | 单独保留，不纳入选定的主实验方法和上述 18 个配置 |
...
| 作者提出的更正值 | Llama CA 准确率统一加 8 个百分点，见第 12 节 | 未核验的算术调整，非实测；不计入主实验或消融数量 |
...
| CORAL-Wikipag | 使用 Wikipag 的 CORAL 适配实现；已核对的双模型启动配置是一轮检索，包含双查询与答案门控；Qwen 使用 raw-Wikipag no-fallback 版本，见第 3.1 节。均不能直接称为原论文完整复现 |
...
以下 12 个配置均有覆盖 47,815 个低资源实例的归档汇总。新增 Qwen CORAL 记为 M12，以保留原有编号；该分支报告只提供综合和分语言结果，两个数据集的单独分数标为“未单列”。

Llama CA 的作者更正请求单列于第 12 节。下表保留能够从现有归档复算的实测分数，未用该调整规则覆盖原记录。
...
| M12 | Qwen3-8B | CORAL-Wikipag（raw / no-fallback） | 未单列 | 未单列 | 48.87 | 23,366 |

证据：M01 见[零样本汇总][zero-json]；M02 见[tCRAG 汇总][trag-json]；M03 见[Qwen CA 对比汇总][ca-json]；M04-M11 见[双模型汇总][two-json]。

M12 见[Qwen 分支报告][qwen-branch-report]与[分支汇总 JSON][qwen-branch-json]。此前仅检查 `main`，漏查了该分支；Qwen CORAL 已有结果，不应列为尚未运行的实验。其设置为仅检索原始 Wikipag 文段，不使用离线用法卡片库，关闭 D-RAG 回退，并保留直接答案门控。分支中的零样本和 tCRAG 总计数及分语言正确题数与 M01、M02 一致，不重复计数。

Qwen CA 相对该 CORAL 配置提高 **4.62 个百分点**，tCRAG 相对该 CORAL 配置提高 **1.47 个百分点**，均从整数正确题数计算。分支没有提交原始预测，也未单列按数据集的统计；这里核对的是报告汇总，不是逐题复核。上表不代表所有模型都优于所有基线。
...
| M12 | Qwen3-8B / CORAL-Wikipag（raw / no-fallback） | 51.16 | 52.68 | 50.90 | 39.18 | 50.41 |
...
| M12 | Qwen3-8B / CORAL-Wikipag（raw / no-fallback） | 1 | 100.00 |

M12 的无效数由分支报告的 `parse_rate` 与 47,815 个实例反算为 1；其有效率四舍五入为 100.00%，并非所有输出都有效。其他数量来自 `parsed`/`valid` 汇总。已审查的部分汇总器信任存储标记，并将缺失标记默认视为有效；本清单没有独立重新解析全部原始答案。有效率高不等于答案解析一定正确。
...
### 5.6 分支归档中的 D-RAG 历史对照

[Qwen 分支报告][qwen-branch-report]还记录了一个 `D-RAG adaptive` 配置，模型为 Qwen3-8B，共 47,815 个实例。报告描述为多语言 Wikipag 检索加评分门控，按此适配版本保留名称，不称为原论文完整复现。

| 编号 / 方法 | 正确题数 | 综合准确率 | bn | hi | ne | sw | te |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| S01 / D-RAG adaptive | 20,592 | 43.07 | 44.83 | 46.78 | 44.98 | 35.96 | 42.78 |

该运行作为已有历史对照归档，不加回当前选定的零样本、tCRAG、CORAL 和 CA 主表。它不包含在主实验与运行消融的 18 个配置内；原始预测和按数据集拆分同样未随该分支报告提交。
...
| 其他讨论过的方法与训练方案 | D-RAG adaptive 汇总已在分支中定位，见第 5.6 节；DKM-RAG、QTT-RAG、MultiRAG 和当前 SFT/GRPO 尚未定位完整可核验结果 | 对话中讨论、已有代码或旧稿文字都不等于已完成实验 |

## 10. 论文中可以怎样引用这份清单

| 论文用途 | 应引用的内容 | 不应混入的内容 |
| --- | --- | --- |
| 主实验 | M01-M12，明确模型、基准与实现适配 | 不要推算 Qwen CORAL 未单列的分数据集分数，或混入旧稿训练版本数字 |
...
总结：**当前已归档的低资源主实验与运行消融共有 18 个不重复配置；另外有 D-RAG 历史配置、高资源语言补充评测、提示历史对照与运行记录、全库分析及理论推导。内容表示、构建流程与人工质量等旧稿条目仍需追溯，不能算入当前版本已完成的实测证据。**

## 11. 来源索引与本次核验范围

本清单的主要数值来自 8 份远程归档 JSON：原有零样本、tCRAG、Qwen CA 合并汇总、双模型合并汇总、消融合并汇总及两个 Qwen CA 运行分片，加上 Qwen 分支的多语言 RAG 汇总。原有 7 份缓存逐一核对 Git blob SHA-1；新增汇总直接读取固定分支提交的 Git blob 并校验。所有新增表格分数按整数正确题数重算，同时核对分语言正确题数之和与总数一致。
...
- 分支补充证据：[Qwen 多语言 RAG 报告][qwen-branch-report]、[JSON 汇总][qwen-branch-json]，包含 Qwen CORAL 与 D-RAG adaptive。
- 远程可读报告：[Qwen tCRAG][trag-report]、[Qwen CA][ca-report]、[Qwen 消融][ablation-report]、[双模型主实验][two-report]。

## 12. Llama CA 作者更正请求（未核验，非实测）

作者提出 Llama-3.1-8B 的 CA 准确率应统一增加 8 个百分点，并表示对应结果记录可能已丢失。以下数值仅按“现有展示准确率 + 8.00 个百分点”换算，记录这一更正请求；它们不是重新运行或重算预测得到的成绩，也不是理论推导的预期值。

| Llama CA 指标 | 当前归档准确率（%） | 作者提出的调整值（%，未核验） |
| --- | ---: | ---: |
| Global-MMLU | 41.99 | 49.99 |
| MMLU-ProX | 27.77 | 35.77 |
| 综合 | 34.23 | 42.23 |
| bn（两数据集合并） | 34.61 | 42.61 |
| hi（两数据集合并） | 37.58 | 45.58 |
| ne（两数据集合并） | 35.02 | 43.02 |
| sw（两数据集合并） | 31.33 | 39.33 |
| te（两数据集合并） | 32.59 | 40.59 |

已重新比对现有归档两个 Llama CA 分片的 `prediction` 与 `answer`：共 47,815 个唯一实例，正确 16,366 个，与存储的 `correct` 标记一致，准确率为 34.23%。这一检查只确认现有归档的内部一致性，不能确认或恢复另一轮已丢失的结果。

调整值不替代第 3 节主表，不推算新的正确题数，不用于声称“Llama 上 CA 最好”，也不改变有效率、显著性结论或实验配置数量。若找回对应运行记录，再从逐题数据重新生成汇总并统一更新主表与结论。

[qwen-branch-report]: https://github.com/Traveler03/CA/blob/cb0d1b2a3ae9a8523264f814a3c960f5e256809f/reports/full_multilingual_rag_qwen3_8b_results_20260811.md
[qwen-branch-json]: https://github.com/Traveler03/CA/blob/cb0d1b2a3ae9a8523264f814a3c960f5e256809f/reports/full_multilingual_rag_qwen3_8b_results_20260811.json
